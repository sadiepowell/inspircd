...
public:
	...
	ExtendedISupportCap(const WeakModulePtr& mod)
...
public:
	...
	CommandISupport(const WeakModulePtr& mod)
	...
	CmdResult HandleLocal(LocalUser* user, const Params& parameters) override
	{
		...
		{
			user->WriteNumeric(ERR_UNKNOWNCOMMAND, this->service_name, FMT::format("You need the {} client capability to use this command",
...
public:
	ModuleExtendedISupport()
		: Module(VENDOR, "Provides support for the DRAFT IRCv3 Extended ISupport specification.")
		, ClientProtocol::EventHook(weak_from_this(), "NUMERIC")
		, ISupport::EventListener(weak_from_this())
		, batch("draft/extended-isupport")
		, batchmanager(weak_from_this())
		, isupportcmd(weak_from_this())
	{
	}
